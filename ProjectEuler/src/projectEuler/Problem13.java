...
public class Problem13 extends EulerProblem {

	public Problem13(String name, String description) {
	...
	public String Solve() {
		...
		LinkedList<Integer> finalNumbers = new LinkedList<Integer>();

		int carryOver = 0;
		for(int magnitude=numbers[0].length - 1;magnitude>=0;magnitude--) {
			int magSum = carryOver;
			for(int number=0;number<numbers.length;number++) {
				magSum += numbers[number][magnitude];
			}
			int currentPlace = magSum%10;
			finalNumbers.push(currentPlace);
			carryOver = (magSum - currentPlace) / 10;
		}
		while(carryOver > 0) {
			finalNumbers.push(carryOver%10);
			carryOver /= 10;
		}
		
		StringBuilder result = new StringBuilder();
		for(int i=0;i<10;i++) {
			result.append(finalNumbers.pop());
		}
